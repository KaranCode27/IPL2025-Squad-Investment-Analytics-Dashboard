...
print("Total in excel:", len(excel_players))
print("Total extracted:", len(all_extracted))
...
for ep in excel_players:
    ...
    if ep in all_extracted:
        continue
    # try clean names of spaces or minor differences
    found = False
    cleaned_ep = ep.replace(' ', '').lower()
    for name, data in all_extracted.items():
        if name.replace(' ', '').lower() == cleaned_ep:
            found = True
            # print mapping
            print(f"Fuzzy mapped: '{ep}' -> '{name}'")
            break
    if not found:
        missing.append(ep)

print("\nMissing in extracted list:", len(missing))
print(missing)
